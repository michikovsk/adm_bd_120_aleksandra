...
select f.titulo, f.genero from atuacoes at
inner join filmes f
on f.id_filme = at.id_filme;
...
SELECT distinct at.nome_autor, at.nacionalidade
FROM atores at
INNER JOIN atuacoes a ON at.id_autor = a.id_autor
...
select nome_autor, f.titulo 
from atores at
inner join atuacoes a on at.id_autor = a.id_autor
inner join fimes f on a.id_fime = f.id_filme
where f.ano_lancamento like < 2010;
